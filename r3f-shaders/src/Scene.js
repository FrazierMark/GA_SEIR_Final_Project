...
import { Environment, OrbitControls, Sky, Stars, Sparkles } from "@react-three/drei";
// import { mergeBufferGeometries } from
import Wave from "./components/WaveShaderMaterial";
import { mergeBufferGeometries } from "three/examples/jsm/utils/BufferGeometryUtils";
import { BoxGeometry } from "three";
import { Vector2 } from "three";
import { waveShaderMaterial } from "./components/WaveShaderMaterial"
...
const makeHex = (height, position) => {
    ...
    if (height > STONE_HEIGHT) {
        ...
        if (Math.random() > 0.8) {
            grassGeometries = mergeBufferGeometries([grassGeometries, getTreeGeometry(height, position)]);
        }
        // Merging smallStone geometry with the large stone pillars
        if (Math.random() > 0.8) {
            stoneGeometries = mergeBufferGeometries([stoneGeometries, getSmallStoneGeometry(height, position)]);
        }
...
const HexagonGrid = (props) => {
    ...
    const [dirt, dirt2, grass, sand, water, stone] = useLoader(THREE.TextureLoader, [
    ...
    return (
        <>
            ...
            <mesh geometry={sandGeometries} castShadow={true} receiveShadow={true} >
                <meshStandardMaterial envMap={envMap} envMapIntensity={1.135} flatShading={true} map={sand} />
                {/* <waveShaderMaterial uColor={"hotpink"} ref={ref} uTexture={image} /> */}
            </mesh>
            <mesh geometry={grassGeometries} castShadow={true} receiveShadow={true} >
                <meshStandardMaterial envMap={envMap} envMapIntensity={1.135} flatShading={true} map={grass} />
                {/* <waveShaderMaterial uColor={"hotpink"} ref={ref} uTexture={image} /> */}
            </mesh>
            <mesh geometry={dirtGeometries} castShadow={true} receiveShadow={true} >
                <meshStandardMaterial envMap={envMap} envMapIntensity={1.135} flatShading={true} map={dirt} />
                {/* <waveShaderMaterial uColor={"hotpink"} ref={ref} uTexture={image} /> */}
            </mesh>
            <mesh geometry={stoneGeometries} castShadow={true} receiveShadow={true} >
                <meshStandardMaterial envMap={envMap} envMapIntensity={1.135} flatShading={true} map={stone} />
                {/* <waveShaderMaterial uColor={"hotpink"} ref={ref} uTexture={image} /> */}
                <Tree />
            </mesh>
        </>
...
const Container = () => {
    ...
    const dirt = useLoader(THREE.TextureLoader, 'https://makio135.com/matcaps/64/6E524D_8496C5_AF6624_100B11-64px.png')
    return (
...
const Scene = () => {
    return (
        <Canvas shadows camera={{ near: 0.01, far: 190, fov: 45, position: [-17, 43, 33] }} gl={{ alpha: false, antialias: false }} dpr={[1, 1.5]} >
            {/* <Stars radius={10} depth={50} count={5000} factor={4} saturation={0} fade speed={1} /> */}
            <Sky azimuth={0.1} turbidity={10} rayleigh={0.5} inclination={0.6} distance={500} />
            ...
            <OrbitControls dampingFactor={.5} enableDamping="true" target={[0, 0, 0]} />
            <Suspense fallback={null}>
                <Wave />
            </Suspense>
        </Canvas>
